...
# VR-controller pinch depth ratios (analogue of Brainco's PINCH_RATIOS).
# For inspire hand: q command 1.0 = fully open, 0.0 = fully closed, so a mapped
# joint closes to q = 1.0 - press * PINCH[...] at full trigger press.
# Values below are URDF-FK verified: at full press thumb_tip ≈ index_tip (~2mm),
# i.e. a precision pinch. Do NOT use 1.0 for all — that overshoots the opposition
# (thumb rotates past the index) and curls the index too far inward.
#   idx5 thumb-rotation: yaw→1.10 rad (63°) → depth 0.857
#   idx4 thumb-bend:     pitch→0.25 rad     → depth 0.500
#   idx3 index:          q→0.90 rad         → depth 0.529
PINCH = {
    5: 0.857,  # thumb-rotation (opposition)
    4: 0.500,  # thumb-bend (flexion)
    3: 0.529,  # index (flexion)
}


def _controller_to_inspire_q(left_trigger_value, left_squeeze_value, right_trigger_value, right_squeeze_value):
    """
    Map XR controller trigger/squeeze -> inspire hand q targets (1.0 open, 0.0 closed).
    Trigger -> precision pinch (thumb-rotation 5, thumb-bend 4, index 3);
    squeeze -> power grasp (middle 2, ring 1, pinky 0). Joint order:
    pinky(0), ring(1), middle(2), index(3), thumb-bend(4), thumb-rotation(5).
    """
    left_q_target  = np.full(Inspire_Num_Motors, 1.0, dtype=float)
    right_q_target = np.full(Inspire_Num_Motors, 1.0, dtype=float)

    # trigger: [10→0] → press [0→1]
    lt = (10.0 - left_trigger_value) / 10.0
    rt = (10.0 - right_trigger_value) / 10.0

    for t, q in ((lt, left_q_target), (rt, right_q_target)):
        # eased thumb opposition curve (mirrors Brainco), then close = 1.0 - press*depth
        q[5] = 1.0 - (1.0 - (1.0 - t) ** 2) * PINCH[5]  # thumb-rotation (opposition)
        q[4] = 1.0 -  t                     * PINCH[4]  # thumb-bend (flexion)
        q[3] = 1.0 -  t                     * PINCH[3]  # index (flexion)

    # squeeze: [0→1] → power grasp on middle/ring/pinky
    for s, q in ((left_squeeze_value, left_q_target), (right_squeeze_value, right_q_target)):
        q[2] = 1.0 - s  # middle
        q[1] = 1.0 - s  # ring
        q[0] = 1.0 - s  # pinky
...
class Inspire_Controller_DFX:
    def __init__(self, left_hand_array, right_hand_array, dual_hand_data_lock = None, dual_hand_state_array = None,
        ...
        wait_for_dds(lambda: self.hand_sub_ready, "Inspire_Controller_DFX")
...
class Inspire_Controller_DFX_ctrl:
    """
    Inspire DFX hand driven by XR controller trigger + squeeze (no hand tracking).
    Mirrors Brainco_Controller_ctrl, adapted to inspire's polarity (1.0 = open, 0.0 = closed).
    """
    def __init__(self, left_gripper_trigger_in, left_gripper_squeeze_in, right_gripper_trigger_in, right_gripper_squeeze_in,
                       dual_hand_data_lock = None, dual_hand_state_array = None, dual_hand_action_array = None, fps = 100.0,
                       Unit_Test = False, simulation_mode = False, xr_motion_data_ready_in = None):
        ...
        wait_for_dds(lambda: self.hand_sub_ready, "Inspire_Controller_DFX_ctrl")
    ...
    def control_process(self, left_gripper_trigger_in, left_gripper_squeeze_in, right_gripper_trigger_in, right_gripper_squeeze_in,
        ...
        # 1.0 = fully open (inspire idle state)
        left_q_target  = np.full(Inspire_Num_Motors, 1.0, dtype=float)
        right_q_target = np.full(Inspire_Num_Motors, 1.0, dtype=float)
        ...
        try:
            while self.running:
                ...
                if xr_motion_data_ready:
                    left_q_target, right_q_target = _controller_to_inspire_q(
                        left_trigger_value, left_squeeze_value, right_trigger_value, right_squeeze_value)
...
class Inspire_Controller_FTP:
    def __init__(self, left_hand_array, right_hand_array, dual_hand_data_lock = None, dual_hand_state_array = None,
        ...
        wait_for_dds(lambda: self.hand_sub_ready, "Inspire_Controller_FTP")
    ...
    def _subscribe_hand_state(self):
        ...
        left_state_ready = False
        right_state_ready = False
        while True:
            # Left Hand
            ...
            if left_state_ready and right_state_ready:
                self.hand_sub_ready = True
            time.sleep(0.002)
...
class Inspire_Controller_FTP_ctrl:
    """
    Inspire FTP hand driven by XR controller trigger + squeeze (no hand tracking).
    Mirrors Brainco_Controller_ctrl, adapted to inspire's polarity (1.0 = open, 0.0 = closed).
    """
    def __init__(self, left_gripper_trigger_in, left_gripper_squeeze_in, right_gripper_trigger_in, right_gripper_squeeze_in,
                       dual_hand_data_lock = None, dual_hand_state_array = None, dual_hand_action_array = None, fps = 100.0,
                       Unit_Test = False, simulation_mode = False, xr_motion_data_ready_in = None):
        logger_mp.info("Initialize Inspire_Controller_FTP_ctrl...")
        from inspire_sdkpy import inspire_dds, inspire_hand_defaut # lazy import
        ...
        wait_for_dds(lambda: self.hand_sub_ready, "Inspire_Controller_FTP_ctrl")
        ...
        logger_mp.info("Initialize Inspire_Controller_FTP_ctrl OK!\n")

    def _subscribe_hand_state(self):
        logger_mp.info("[Inspire_Controller_FTP_ctrl] Subscribe thread started.")
        left_state_ready = False
        right_state_ready = False
        while True:
            # Left Hand
            ...
            if left_state_msg is not None:
                if hasattr(left_state_msg, 'angle_act') and len(left_state_msg.angle_act) == Inspire_Num_Motors:
                    ...
                    logger_mp.warning(f"[Inspire_Controller_FTP_ctrl] Received left_state_msg but attributes are missing or incorrect. Type: {type(left_state_msg)}, Content: {str(left_state_msg)[:100]}")
            # Right Hand
            right_state_msg = self.RightHandState_subscriber.Read()
            if right_state_msg is not None:
                if hasattr(right_state_msg, 'angle_act') and len(right_state_msg.angle_act) == Inspire_Num_Motors:
                    ...
                    logger_mp.warning(f"[Inspire_Controller_FTP_ctrl] Received right_state_msg but attributes are missing or incorrect. Type: {type(right_state_msg)}, Content: {str(right_state_msg)[:100]}")
    ...
    def _send_hand_command(self, left_angle_cmd_scaled, right_angle_cmd_scaled):
        ...
        """
        # Left Hand Command
        left_cmd_msg = self._inspire_hand_defaut.get_inspire_hand_ctrl()
        left_cmd_msg.angle_set = left_angle_cmd_scaled
        left_cmd_msg.mode = 0b0001 # Mode 1: Angle control
        self.LeftHandCmd_publisher.Write(left_cmd_msg)

        # Right Hand Command
        right_cmd_msg = self._inspire_hand_defaut.get_inspire_hand_ctrl()
        right_cmd_msg.angle_set = right_angle_cmd_scaled
        right_cmd_msg.mode = 0b0001 # Mode 1: Angle control
        self.RightHandCmd_publisher.Write(right_cmd_msg)
    ...
    def control_process(self, left_gripper_trigger_in, left_gripper_squeeze_in, right_gripper_trigger_in, right_gripper_squeeze_in,
                              left_hand_state_array, right_hand_state_array, dual_hand_data_lock = None, dual_hand_state_array = None, dual_hand_action_array = None,
                              xr_motion_data_ready_in = None):
        logger_mp.info("[Inspire_Controller_FTP_ctrl] Control process started.")
        self.running = True
        ...
        try:
            ...
            logger_mp.info("Inspire_Controller_FTP_ctrl has been closed.")
