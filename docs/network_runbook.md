...
键盘 `R/S/Q` 保留为备用。机器人使用 G1 官方 `voice` 服务，默认依次用普通话和英文播报系统准备、手柄按键、采集、保存和退出状态；中文播报将 A/B/Y 读作“诶键、比键、歪键”。启动前可设置 `VOICE_LANGUAGE=zh`、`VOICE_LANGUAGE=en` 或 `VOICE_LANGUAGE=bilingual`，也可追加 `--no-voice` 完全关闭语音。按键使用上升沿触发，长按不会重复切换。
